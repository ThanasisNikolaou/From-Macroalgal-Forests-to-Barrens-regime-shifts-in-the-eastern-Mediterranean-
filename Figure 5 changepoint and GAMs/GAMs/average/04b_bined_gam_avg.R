...
# Load data
dataset <- read.delim("clipboard") # read the average values

# Shapiro-Wilk test for normality
shapiro.test(dataset$canopy)
hist(dataset$canopy)

gam_model <- gam(canopy ~ s(CH, k=4), 
                 family = tw(link = "log"), 
                 data = dataset, method = "REML")
summary(gam_model)

# # # # # DHARMA Diagnostics # # # # # #
png("DHARMa_main_diagnostics.png",
...
# # # # # # DHARMA uniformity # # # # # #
png("DHARMa_uniformity_test.png",
...
# # # # # # DHARMA dispersion # # # # # #
png("DHARMa_dispersion_test.png",
    width = 2400, height = 1800, res = 300)
...
# Plot
p_gam <- ggplot() +
  geom_point(data = dataset, aes(x = CH, y = canopy),
...
ggsave(
  filename = "GAM_canopy_vs_CHi.png",
  plot = p_gam,
  width = 8,
  height = 6,
  dpi = 300
...
p_gam_2 = ggplot() +
  ...
  # Actual data points
  geom_point(data = dataset, aes(x = CH, y = canopy), color = "red", size = 2, alpha = 0.7) +
  
  coord_cartesian(ylim = c(0, 60)) +
  
  # Labels and theme
  labs(x = "Cumulative Herbivory Index (g consumed per day)", 
       y = "Average canopy cover (%)",
       title = "") +
  theme_classic(base_size = 14) +
  theme(
    plot.title = element_text(hjust = 0.5, size = 16, face = "bold"),
